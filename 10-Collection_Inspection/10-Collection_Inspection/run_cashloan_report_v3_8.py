...
def make_collection_performance_section(perf_data):
    ...
    for bucket, data in groups.items():
        ...
        def _render_row(item):
            ...
            d_color = "text-green-600" if delta > 0 else "text-red-500"
            return f"""
            <div class="flex justify-between items-center text-xs py-1 border-b last:border-0">
                <span class="truncate w-1/2" title="{item['group_name']}">{item['group_name']}</span>
                <span class="font-mono">{rr:.1%}</span>
                <span class="font-bold {d_color} w-12 text-right">{delta:+.1%}</span>
            </div>
            """
        ...
        if is_short:
             ...
             top3 = all_list[:3]
             bot3 = all_list[-3:]
             content_html += "<div class='mb-2 text-xs font-bold text-gray-400 uppercase'>Top 3</div>"
             content_html += "".join([_render_row(x) for x in top3])
             content_html += "<div class='mt-2 mb-2 text-xs font-bold text-gray-400 uppercase'>Bottom 3</div>"
        ...
        <div class="mb-4 border rounded p-4 bg-white shadow-sm">
            <div class="flex justify-between mb-2">
                ...
                <span class="text-xs text-gray-400">BM: {bm_rate:.1%}</span>
            </div>
            <div class="flex justify-between text-xs text-gray-400 px-1 mb-1 border-b pb-1">
                <span>Group</span>
                <span>Rate</span>
                <span>Vs M-1</span>
            </div>
    ...
    <div class="card">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="col-span-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {html_groups}
            </div>
        </div>
    </div>
...
<head>
    ...
    <title>CashLoan Inspection Report {date_str} (v3.8)</title>
...
<body class="p-6 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">CashLoan Inspection Report v3.8</h1>
