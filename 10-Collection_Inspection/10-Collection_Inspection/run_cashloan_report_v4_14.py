...
<head>
    ...
    <title>CashLoan Inspection Report {date_str} (v4.14)</title>
...
<body class="p-6 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">CashLoan Inspection Report v4.14</h1>
    ...
    <!-- ═══════════════ Part 1: 回收监控 ═══════════════ -->
    <div class="section-title" id="part1"><span>1</span>回收监控 (Recovery Monitoring)</div>

    <!-- 1.1 Due Month Recovery Curve -->
    {html_dm_recovery}

    <!-- 1.2 Term Monitoring Matrix -->
    {html_term_matrix}

    <!-- ═══════════════ Part 2: 资产质量 & 归因 ═══════════════ -->
    <div class="section-title" id="part2"><span>2</span>资产质量 & 归因 (Asset Quality & Attribution)</div>
    ...
    <div class="text-center text-slate-400 text-sm mt-8 pb-8">
        <p>Report System v4.14 | Maintainer: <strong>Mr. Yuan</strong></p>
    </div>
    ...
    <script>
        ...
        /* [v4.14] Top Nav: click-to-scroll + ScrollSpy */
        ...
        (function() {{
            var partIds = ['part1', 'part2', 'part3'];
            var navItems = document.querySelectorAll('.top-nav-item[data-nav]');

            function updateNav() {{
                var scrollY = window.scrollY + 80;
                var current = partIds[0];
                for (var i = 0; i < partIds.length; i++) {{
                    var el = document.getElementById(partIds[i]);
