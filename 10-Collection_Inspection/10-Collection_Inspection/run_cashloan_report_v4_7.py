# -*- coding: utf-8 -*-
"""
CashLoan 巡检日报 v4.7：基于 v4.2 稳定版增量开发。
Updates:
- v4.7: Term Monitoring Matrix (到期月×MOB1~4, 新老客/期数筛选)。
- v4.7: 回收归因兜底展示。
- v4.7: 放款账龄归因 (到期月×MOB)。
- 基线 (v4.2): MTD Lift, Vintage Matrix, Risk Attribution, Amount Heatmap, Contactability, Collection Performance。
"""
import sys
import json
import os
                ...
                if prev_r:
                    ...
                    if vp is not None:
                        ...
                        if abs(delta) > 0.001:
                            ...
                            trend_html = f"<span class='text-[9px] {color} ml-0.5'>{arrow}</span>"
...
def make_term_monitoring_section(term_data):
    """
    [v4.7 New] 期限监控矩阵 (Term Monitoring Matrix).
    纵轴: 到期月, 横轴: MOB1~4, 支持新老客/期数筛选。
    """
    if not term_data or not term_data.get("months"):
        return ""
    ...
    for i, ut in enumerate(user_types):
        label = "All" if ut == "all" else str(ut)
        active = "active" if i == 0 else ""
        ut_btns += f'<button onclick="switchTermFilter(\'ut\', \'{ut}\')" class="tab-btn {active}" id="btn-term-ut-{ut}">{label}</button>'
    
    ps_btns = ""
    for i, ps in enumerate(period_seqs):
        label = "All" if ps == "all" else f"{ps}期"
        active = "active" if i == 0 else ""
        ps_btns += f'<button onclick="switchTermFilter(\'ps\', \'{ps}\')" class="tab-btn {active}" id="btn-term-ps-{ps}">{label}</button>'
    
    # Build all combo tables (hidden by default, shown via JS)
    tables_html = ""
    for ut in user_types:
        for ps in period_seqs:
            key = (ut, ps)
            combo_data = data.get(key, {})
            combo_id = f"term-{ut}-{ps}"
            hidden = "" if ut == user_types[0] and ps == period_seqs[0] else "hidden"
            
            # Header
            header_cells = "".join(f"<th class='px-3 py-2 text-center'>MOB{m}</th>" for m in mob_cols)
            
            # Rows
            rows_html = ""
            for month in months:
                month_data = combo_data.get(month, {})
                cells = ""
                for m in mob_cols:
                    ...
                    if val is not None:
                        ...
                        cells += f'<td class="px-3 py-1.5 text-center text-sm {color}">{pct}</td>'
                    else:
                        cells += '<td class="px-3 py-1.5 text-center text-sm text-slate-300">-</td>'
                rows_html += f'<tr class="border-b border-slate-100 hover:bg-slate-50"><td class="px-3 py-1.5 text-sm font-medium text-slate-700">{month}</td>{cells}</tr>'
            
            if not rows_html:
                rows_html = f'<tr><td colspan="{len(mob_cols)+1}" class="text-center text-sm text-slate-400 py-4">暂无数据</td></tr>'
            
            tables_html += f"""
            <div id="{combo_id}" class="term-combo {hidden}">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead><tr class="bg-slate-50 border-b-2 border-slate-200">
                            <th class="px-3 py-2 text-sm font-semibold text-slate-600">到期月</th>
                            {header_cells}
                        </tr></thead>
    ...
    <div class="card">
        <div class="flex items-center justify-between mb-4 border-b pb-4">
            <div class="flex items-center gap-2">
                <div class="section-bar accent"></div>
                <h2 class="text-lg font-bold text-slate-800">期限监控矩阵 (Term Monitoring Matrix)</h2>
            </div>
            <div class="flex space-x-4">
                <div class="flex space-x-2 bg-slate-100 p-1 rounded-lg">{ut_btns}</div>
                <div class="border-l pl-4 flex space-x-2 bg-slate-100 p-1 rounded-lg">{ps_btns}</div>
            </div>
        </div>
        <p class="text-xs text-slate-400 mb-3">纵轴: 到期月 | 横轴: MOB (放款账龄) | 指标: 入催率 (Entrant Rate) | 筛选: 新老客 × 产品期数</p>
        {tables_html}
    </div>
    """
...
<head>
    ...
    <title>CashLoan Inspection Report {date_str} (v4.7)</title>
...
<body class="p-6 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">CashLoan Inspection Report v4.7</h1>
    ...
    <!-- 1.7 Term Monitoring Matrix (v4.7) -->
    {html_term_matrix}
    ...
    <div class="text-center text-slate-400 text-sm mt-8 pb-8">
        <p>Report System v4.7 | Maintainer: <strong>Mr. Yuan</strong></p>
    </div>
    ...
    <script>
        ...
        // Term Monitoring Matrix Filter Logic (v4.7)
        ...
        function switchTermFilter(dim, val) {{
            ...
            document.querySelectorAll('.term-combo').forEach(el => el.classList.add('hidden'));
            const targetId = 'term-' + currentTermUt + '-' + currentTermPs;
