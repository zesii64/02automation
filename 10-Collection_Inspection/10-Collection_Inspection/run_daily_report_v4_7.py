...
def compute_term_monitoring_matrix(df):
    ...
    纵轴: 到期月 (due_date -> YYYY-MM)
    横轴: MOB1, MOB2, MOB3, MOB4 (period_no = 1~4)
    指标: entrant_rate (overdue / owing)
    支持筛选: user_type (新老客), period_seq (1期/3期/6期)
    
    Returns: {
        "months": ["2026-01", "2025-12", ...],
        "mob_cols": [1, 2, 3, 4],
        "data": {
            ("all", "all"): { "2026-01": {1: 0.15, 2: 0.12, ...}, ... },
            ("新客", "all"): { ... },
            ("all", 1): { ... },
            ...
        },
        "user_types": ["all", "新客", "老客"],
        "period_seqs": ["all", 1, 3, 6]
    }
    """
    if df is None or df.empty:
        return {}
    ...
    df["due_month"] = df["due_date"].dt.strftime("%Y-%m")
    
    # Determine available MOB columns (1~4)
    mob_cols = sorted([v for v in df["period_no"].dropna().unique() if v in [1, 2, 3, 4]])
    if not mob_cols:
        mob_cols = [1, 2, 3, 4]
    
    # Determine available user_types and period_seqs
    user_types = ["all"]
    ...
    if "period_seq" in df.columns:
        available_seqs = sorted(df["period_seq"].dropna().unique().tolist())
        # Only include common product terms
        for s in [1, 3, 6]:
            if s in available_seqs:
                period_seqs.append(s)
    
    # Build matrix for each (user_type, period_seq) combo
    result_data = {}
    ...
    for ut in user_types:
        for ps in period_seqs:
            # Filter
            sub = df.copy()
            if ut != "all" and "user_type" in sub.columns:
                sub = sub[sub["user_type"] == ut]
            if ps != "all" and "period_seq" in sub.columns:
                sub = sub[sub["period_seq"] == ps]
            
            if sub.empty:
                continue
            
            # Group by (due_month, period_no) -> calc entrant_rate
            grouped = sub.groupby(["due_month", "period_no"]).agg(
            ...
            for _, row in grouped.iterrows():
                month = row["due_month"]
                mob = int(row["period_no"])
                if mob not in mob_cols:
                    continue
                owing = row["owing"]
                overdue = row["overdue"]
                rate = round(overdue / owing, 4) if owing > 0 else None
                
                if month not in matrix:
                    matrix[month] = {}
                matrix[month][mob] = rate
            
            if matrix:
                result_data[(ut, ps)] = matrix
    
    # All months sorted DESC
    all_months = sorted(set(m for combo_data in result_data.values() for m in combo_data.keys()), reverse=True)
