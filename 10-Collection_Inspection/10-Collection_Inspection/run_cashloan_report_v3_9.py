...
def make_attribution_content(attr_data_single):
    ...
    for dim_name, rows in dims.items():
        ...
        for r in top_rows:
            ...
            # [v3.8] Connect Rate
            conn_rate = r.get("curr_conn_rate")
            conn_html = f"{conn_rate:.1%}" if conn_rate is not None else "-"
            ...
            <tr class="border-b hover:bg-slate-50">
                ...
                <td class="p-2 text-right text-xs text-blue-500">{conn_html}</td>
                ...
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-100 text-xs uppercase text-gray-500">
                        <tr>
                            ...
                            <th class="p-2 text-right">Conn %</th>
...
def make_collection_performance_section(perf_data):
    ...
    for bucket, data in groups.items():
        ...
        def _render_row(item):
            ...
            # Process Metrics
            cov = item.get('cov', 0)
            cov_d = item.get('cov_delta', 0)
            
            conn = item.get('conn', 0)
            conn_d = item.get('conn_delta', 0)
            
            int_val = item.get('int', 0)
            int_d = item.get('int_delta', 0)
            
            def fmt_proc(val, delta):
                if val == 0: return "-"
                # Color delta only if significant
                c_cls = "text-gray-400"
                if delta > 0.02: c_cls = "text-green-500"
                if delta < -0.02: c_cls = "text-red-500"
                # Arrow
                arr = "↑" if delta > 0 else "↓"
                d_str = f"<span class='{c_cls} text-[10px]'>{arr}{abs(delta):.0%}</span>" if abs(delta)>0.001 else ""
                return f"{val:.0%} {d_str}"

            p_cov_html = fmt_proc(cov, cov_d)
            p_conn_html = fmt_proc(conn, conn_d)
            ...
            <div class="grid grid-cols-12 gap-1 items-center text-xs py-2 border-b last:border-0 hover:bg-slate-50">
                ...
                <div class="col-span-2 text-right font-bold {d_color}">{delta:+.1%}</div>
                <div class="col-span-2 text-right text-slate-500 text-[11px]">{p_cov_html}</div>
                <div class="col-span-3 text-right text-slate-500 text-[11px]">{p_conn_html}</div>
            </div>
            """
        ...
        <div class="grid grid-cols-12 gap-1 text-[10px] text-gray-400 uppercase border-b pb-1 mb-1">
            ...
            <div class="col-span-2 text-right">Vs M-1</div>
            <div class="col-span-2 text-right">Cov%</div>
            <div class="col-span-3 text-right">Conn%</div>
        </div>
        """
        ...
        if is_short:
             ...
             top3 = all_list[:3]
             bot3 = all_list[-3:]
             content_html += "<div class='mb-1 mt-2 text-[10px] font-bold text-gray-300 uppercase'>Top 3</div>"
             content_html += "".join([_render_row(x) for x in top3])
             content_html += "<div class='mb-1 mt-2 text-[10px] font-bold text-gray-300 uppercase'>Bottom 3</div>"
        ...
        <div class="mb-4 border rounded p-4 bg-white shadow-sm">
            <div class="flex justify-between mb-2">
                ...
                <div class="text-xs text-right">
                    <div class="text-gray-500">BM: {bm_rate:.1%}</div>
                    <div class="text-[10px] text-gray-400">Avg Cov: {p_bm.get('cov',0):.0%} | Conn: {p_bm.get('conn',0):.0%}</div>
                </div>
            </div>
            <div class="space-y-0">
                {content_html}
    ...
    <div class="card">
        <div class="flex items-center gap-2 mb-4 border-b pb-4">
            <div class="section-bar accent"></div>
            <h2 class="text-lg font-bold text-slate-800">催收团队效能 (Collection Org Performance)</h2>
        </div>
...
<head>
    ...
    <title>CashLoan Inspection Report {date_str} (v3.9)</title>
...
<body class="p-6 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">CashLoan Inspection Report v3.9</h1>
    ...
    <!-- 2.6 Collection Performance (v3.7) -->
    {html_perf}
