# -*- coding: utf-8 -*-
"""
全链路巡检脚本 (All-in-One Inspection Script) v4.22
Feature:
...
SELECT day
     ...
     , SUM(start_owing_principal)    AS start_owing_principal
FROM phl_anls.tmp_maoruochen_phl_repay_natural_day_daily
WHERE TO_CHAR(dt_biz, 'yyyyMM') >= '202511'
GROUP BY day
...
def download_data(output_file="collection_inspection_data_local.xlsx"):
    ...
    tasks = [
        ...
        ("natural_month_repay_daily", SQL_REPAY_DAILY),   # [v4.10 NEW] 日粒度序时
        ("due_month_repay", SQL_DUE_MONTH_REPAY),         # [v4.10 NEW] 到期月回收
    ]
...
def generate_report(excel_path):
    ...
    try:
        from run_daily_report_v4_23 import (
            ...
            compute_natural_month_progress,   # [v4.12 multi-level]
            compute_due_month_recovery,       # [v4.11 FIX: user_type only]
            compute_shift_share,              # [v4.17 NEW]
            compute_ops_attribution,          # [v4.21 NEW]
            compute_smart_diagnostics         # [v4.23 NEW]
        )
        from run_cashloan_report_v4_23 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    ...
    out_path = reports_dir / f"CashLoan_Inspection_Report_{date_str}_v4_23.html"
