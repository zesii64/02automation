...
def compute_collection_performance(df, df_process=None):
    ...
    [v3.9 Enhanced] Process Attribution (Merge Process Metrics).
    
    Hierarchy: Agent Bucket -> Group Name -> Owner ID.
    Metrics: Repay Rate, Contribution.
    Process Metrics: Coverage, Connect Rate, Intensity.
    """
    if df is None or df.empty:
        return {}
    ...
    # [v3.9] Prepare Process Data for Current Month
    proc_map = {} # { (bucket, group): {cov:.., conn:.., int:..} }
    proc_benchmarks = {} # { bucket: {cov:.., conn:..} } (Avg)

    if df_process is not None and not df_process.empty:
        # Filter process data to current month
        # Note: SQL_PROCESS natural_month format 'yyyymm' vs 'yyyyMM'? 
        # SQL_REPAY is 'yyyyMM'. SQL_PROCESS is 'yyyymm'.
        # Let's normalize. 
        # Actually in SQL they both seem to output strings. Let's assume matching or try to match.
        
        # Check format
        p_months = df_process["natural_month"].astype(str).unique()
        # Try to match curr_month (which is string like '202601')
        
        # If formats differ (e.g. 202601 vs 2026-01), we need to handle.
        # Assuming they match for now based on previous versions.
        
        df_p_curr = df_process[df_process["natural_month"].astype(str) == str(curr_month)]
        
        if not df_p_curr.empty:
            # Aggregate Process Data by Group
            # SQL already aggregated by Group. Just need to index.
            # But wait, SQL_PROCESS has owner_bucket, owner_group.
            
            # 1. Calc Bucket Benchmarks (Weighted Avg by Case Count?)
            # Or just simple Avg of Groups? Weighted is better.
            # Weights: raw_owing_case_cnt
            
            g_b_p = df_p_curr.groupby("owner_bucket").apply(
                lambda x: pd.Series({
            ...
            proc_benchmarks = g_b_p
            
            # 2. Build Group Map
            for _, row in df_p_curr.iterrows():
                k = (row["owner_bucket"], row["owner_group"])
                proc_map[k] = {
    ...
    out["proc_benchmarks"] = proc_benchmarks # Debug info
    ...
    if "agent_bucket" in df_curr.columns:
        ...
        for bucket in g_bucket["agent_bucket"].unique():
            ...
            if "group_name" in sub.columns:
                ...
                # [v3.9] Merge Process Data & Calculate Process Deltas
                # Bucket Benchmark for Process
                p_bm = proc_benchmarks.get(bucket, {"cov":0, "conn":0, "int":0})
                
                def get_proc_deltas(grp_name):
                    p_data = proc_map.get((bucket, grp_name))
                    if not p_data: return {}
                    return {
                        "cov": p_data["cov"],
                        "conn": p_data["conn"],
                        "int": p_data["int"],
                        "cov_delta": p_data["cov"] - p_bm.get("cov", 0),
                        "conn_delta": p_data["conn"] - p_bm.get("conn", 0),
                        "int_delta": p_data["int"] - p_bm.get("int", 0)
                    }

                # Apply
                proc_info_list = g_grp["group_name"].apply(get_proc_deltas).tolist()
                # Merge into df is tricky, let's just iterate rows later or use join
                # Easiest: convert to records then enrich
                ...
                # Enrich records with process data
                for rec in all_records:
                    p_info = get_proc_deltas(rec["group_name"])
                    rec.update(p_info)
                
                out["groups"][bucket] = {
                    "all": all_records, 
                    "benchmark": bm_rate,
                    "proc_benchmark": p_bm # Send benchmark to frontend for context
                }
                
            # Rank Agents (Global in Bucket)
