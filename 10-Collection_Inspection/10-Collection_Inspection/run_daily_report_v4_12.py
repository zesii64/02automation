...
def compute_natural_month_progress(df_daily):
    """
    [v4.12] 计算自然月回收序时进度数据 — 支持多层级 (模块级 + 大小额).
    
    输入: DataFrame (natural_month_repay_daily sheet)
      - day, natural_month, case_bucket, agent_bucket, data_level,
        repay_principal, start_owing_principal
    
    输出: dict {
        "levels": ["模块级", "大小额"],
        "level_data": {
            "模块级": {
                "buckets": ["M1","M2","S0","S1","S2"],
                "months": [...],
                "target_month": 250003,
                "data": { "S1": { 202511: [...], ... } },
                "summary": { ... }
            ...
            "大小额": {
                "buckets": ["M1_Large","M1_Small","S1_Large",...],
                "months": [...],
                "target_month": 250003,
                "data": { "S1_Large": { ... }, ... },
                "summary": { ... }
    ...
    data_levels = sorted(df_all['data_level'].dropna().unique())
    
    # ------------------------------------------------------------------
    # 定义要计算的层级 (level_name, data_level_filter, groupby_col)
    # ------------------------------------------------------------------
    level_configs = []
    
    # 1) 模块级: data_level 的第一个 (通常 "1.模块层级"), 按 case_bucket 分组
    module_level = data_levels[0] if data_levels else None
    if module_level:
        level_configs.append(("模块级", module_level, "case_bucket"))
    
    # 2) 大小额: data_level = '1.5.大小模块层级', 按 case_bucket 分组
    #    该层级下 case_bucket 值为 S1_Large, S1_Small 等
    #    只保留含 Large/Small 的 bucket (不含 S1, S1_Other 等)
    LARGE_SMALL_LEVEL = None
    for dl in data_levels:
        if '大小' in str(dl) or '1.5' in str(dl):
            LARGE_SMALL_LEVEL = dl
            break
    
    if LARGE_SMALL_LEVEL:
        level_configs.append(("大小额", LARGE_SMALL_LEVEL, "case_bucket"))
    
    # ------------------------------------------------------------------
    # 内部函数: 计算单个层级的数据
    # ------------------------------------------------------------------
    def _compute_level(df_src, dl_filter, group_col, only_large_small=False):
        df = df_src[df_src['data_level'] == dl_filter].copy() if dl_filter else df_src.copy()
        
        if df.empty:
            return {"buckets": [], "months": [], "target_month": 250003, "data": {}, "summary": {}}
        
        # [v4.12] 大小额层级: 只保留含 Large/Small 的 bucket
        if only_large_small and group_col in df.columns:
            mask = df[group_col].astype(str).str.contains('Large|Small', case=False, na=False)
            df = df[mask]
            if df.empty:
                return {"buckets": [], "months": [], "target_month": 250003, "data": {}, "summary": {}}
        
        # 聚合到 (natural_month, group_col, day)
        agg = df.groupby(['natural_month', group_col, 'day'], as_index=False).agg(
            repay_principal=('repay_principal', 'sum'),
            start_owing_principal=('start_owing_principal', 'sum')
        )
        
        buckets = sorted(agg[group_col].dropna().unique())
        all_months = sorted(agg['natural_month'].dropna().unique())
        ...
        for bkt in buckets:
            bkt_data = agg[agg[group_col] == bkt]
            result_data[bkt] = {}
            
            for month in all_months:
                month_data = bkt_data[bkt_data['natural_month'] == month].sort_values('day')
                if month_data.empty:
                    continue
                ...
                for _, row in month_data.iterrows():
                    d = int(row['day'])
                    start_owing = row['start_owing_principal']
                    repay = row['repay_principal']
                    cum_rate = repay / start_owing if start_owing > 0 else 0
                    points.append({"day": d, "cum_rate": round(cum_rate, 6)})
                
                result_data[bkt][int(month)] = points
            
            # 达成率汇总
            if actual_months and TARGET_MONTH in result_data[bkt]:
                latest_month = actual_months[-1]
                if latest_month in result_data[bkt]:
                    actual_points = result_data[bkt][latest_month]
                    target_points = result_data[bkt][TARGET_MONTH]
                    
                    if actual_points:
                        actual_rate = actual_points[-1]['cum_rate']
                        actual_max_day = actual_points[-1]['day']
                        
                        target_rate_at_day = 0
                        target_final_rate = target_points[-1]['cum_rate'] if target_points else 0
                        for tp in target_points:
                            if tp['day'] <= actual_max_day:
                                target_rate_at_day = tp['cum_rate']
                        
                        achieve_pct = actual_rate / target_rate_at_day if target_rate_at_day > 0 else 0
                        
                        summary[bkt] = {
                            "target_rate": round(target_final_rate, 4),
                            "target_rate_at_day": round(target_rate_at_day, 4),
                            "actual_rate": round(actual_rate, 4),
                            "achieve_pct": round(achieve_pct, 4),
                            "actual_day": actual_max_day,
    ...
    for level_name, dl_filter, group_col in level_configs:
        is_large_small = (level_name == "大小额")
        levels_result[level_name] = _compute_level(df_all, dl_filter, group_col, only_large_small=is_large_small)
        level_names.append(level_name)
    
    if not level_names:
        return {}
    
    # 返回结果: 顶层保持向后兼容 (= 第一个层级的数据)
    first = levels_result[level_names[0]]
    return {
        "levels": level_names,
        "level_data": levels_result,
        # 向后兼容
        **first
    }
