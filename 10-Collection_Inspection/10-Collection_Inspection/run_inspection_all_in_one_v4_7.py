# -*- coding: utf-8 -*-
"""
全链路巡检脚本 (All-in-One Inspection Script) v4.7
Feature:
- v4.7: 基于 v4.2 稳定版增量开发。
- v4.7: 期限监控矩阵 (Term Monitoring Matrix): 到期月×MOB1~4, 新老客/期数筛选。
- v4.7: 回收归因兜底展示 (buckets 为空时显示暂无数据)。
- v4.7: 放款账龄归因 (到期月×MOB 维度)。
- v4.7: SQL 增加 period_seq 字段支持产品期数筛选。
- 基线功能 (v4.2): Vintage Matrix + MTD Lift + Risk Attribution + Amount Heatmap + Contactability + Collection Performance。
"""
...
def generate_vintage_sql():
    ...
    # 1. 维度定义 (Dimensions) [v4.7] 增加 period_seq 支持 1期/3期/6期 产品筛选
    dims = [
...
def generate_report(excel_path):
    ...
    try:
        from run_daily_report_v4_7 import (
        ...
        from run_cashloan_report_v4_7 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    except ImportError as e:
        print(f"错误: 缺少 v4.7 依赖脚本 ({e})。")
        return
    ...
    # [v4.7] Term Monitoring Matrix (到期月×MOB1~4)
    ...
    out_path = reports_dir / f"CashLoan_Inspection_Report_{date_str}_v4_7.html"
