...
def build_cashloan_html(vintage_summary, repay_summary, process_summary, vintage_anomalies, repay_anomalies, excel_path, date_str, overview=None, repay_name="natural_month_repay", is_placeholder=False, 
    ...
    # --- Vintage Matrix (Entrant & Recovery) ---
    def make_vintage_matrix(matrix_data, title):
        if not matrix_data:
            return f"<p class='text-gray-400 text-sm'>无 {title} 数据</p>"
        
        # Headers: Due Date, Entrant, D1, D2, D3, D4, D5, D6, D7, D10, D15, D30
        ...
        for r in matrix_data:
            ...
            # Entrant cell
            ent_bg = ""
            if ent > 0.15: ent_bg = "bg-red-100 text-red-800"
            elif ent > 0.10: ent_bg = "bg-orange-50 text-orange-800"
            ent_cell = f"<td class='text-right data-cell {ent_bg} font-semibold'>{ent:.2%}</td>"
            
            # Recovery cells
            rec_cells = []
            for d in show_days:
                val = r["recovery"].get(f"D{d}")
                if val is None:
                    rec_cells.append("<td class='text-right data-cell bg-gray-50 text-gray-300'>-</td>")
                else:
                    rec_cells.append(f"<td class='text-right data-cell'>{val:.1%}</td>")
            
            rows.append(f"<tr><td class='text-gray-700 whitespace-nowrap'>{due}</td>{ent_cell}{''.join(rec_cells)}</tr>")
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-xs border border-gray-200 rounded-lg mb-2">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th class="text-left py-2 px-2 text-gray-700 sticky left-0 bg-gray-50">Due Date</th>
    ...
    matrix_html_all = make_vintage_matrix(matrix_all, "Overall Matrix")
    matrix_html_new = make_vintage_matrix(matrix_new, "New User Matrix")
    matrix_html_old = make_vintage_matrix(matrix_old, "Old User Matrix")
...
<body>
    <div class="container">
        ...
        <!-- Section 2: Vintage Matrix (New!) -->
        <div class="card p-5 mb-5">
            <div class="flex items-center gap-2 mb-4">
                <div class="section-bar accent"></div>
                <h2 class="font-semibold text-gray-900">二、Vintage 矩阵 (Entrant & Recovery)</h2>
            </div>
            <p class="text-xs text-gray-500 mb-3">说明：入催率 = 入催本金/应还本金；Dx = 1 - (Dx本金/入催本金) [催回率]。灰色"-"表示表现期未到。</p>
            
            <div class="flex space-x-2 mb-4 border-b border-gray-100 pb-2">
                <div id="btn-mat-all" class="tab-btn active" onclick="switchTab('mat-all', 'btn-mat-all', 'mat-tabs')">Overall</div>
                <div id="btn-mat-new" class="tab-btn" onclick="switchTab('mat-new', 'btn-mat-new', 'mat-tabs')">新客 (New)</div>
                <div id="btn-mat-old" class="tab-btn" onclick="switchTab('mat-old', 'btn-mat-old', 'mat-tabs')">老客 (Old)</div>
            </div>

            <div id="mat-all" class="tab-content-mat-tabs block">{matrix_html_all}</div>
            <div id="mat-new" class="tab-content-mat-tabs hidden">{matrix_html_new}</div>
            <div id="mat-old" class="tab-content-mat-tabs hidden">{matrix_html_old}</div>
        </div>
        ...
        <div class="card p-5 mb-5">
            <div class="flex items-center gap-2 mb-4">
                <div class="section-bar accent"></div>
                <h2 class="font-semibold text-gray-900">三、维度拆解 (Breakdown)</h2>
            </div>
            <p class="text-xs text-gray-500 mt-2 mb-3">说明：基于最近 7 天 Due 数据与上期对比。</p>
            {asset_content}
        </div>

        <!-- Section 4: Process & Outcome -->
        <div class="grid grid-cols-2 gap-5 mb-5">
            <div class="card p-5">
                <div class="flex items-center gap-2 mb-4">
                    <div class="section-bar"></div>
                    <h2 class="font-semibold text-gray-900">四、策略执行</h2>
                </div>
            ...
            <div class="card p-5">
                <div class="flex items-center gap-2 mb-4">
                    <div class="section-bar"></div>
                    <h2 class="font-semibold text-gray-900">五、回收结果</h2>
                </div>
                <table class="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                    <thead class="bg-gray-50 border-b border-gray-200">
        ...
        <div class="card p-5 mb-5">
            <div class="flex items-center gap-2 mb-4">
                ...
                <h2 class="font-semibold text-gray-900">六、异常数据检查</h2>
            </div>
