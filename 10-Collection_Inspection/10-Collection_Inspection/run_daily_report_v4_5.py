...
v4.5 Upgrade:
- Waterfall Attribution (Mix vs Rate Effect) for Overdue Rate.
...
def compute_waterfall_data(df_curr, df_prev, dimension="user_type"):
    """
    [v4.5 New] Compute Waterfall Attribution Data.
    Decomposes Overall Rate Delta into:
    1. Mix Effect (Structure Shift): Sum((w_curr - w_prev) * (r_prev - R_total_prev))
    2. Rate Effect (Performance Shift): Sum(w_curr * (r_curr - r_prev))
    
    Dimension default: user_type (with normalization).
    Returns: {
        "start_rate": float,
        "end_rate": float,
        "delta": float,
        "mix_effect": [{"name": str, "val": float}, ...],
        "rate_effect": [{"name": str, "val": float}, ...]
    }
    """
    ...
    if dimension not in df_curr.columns:
        if dimension == "user_type" and "user_group" in df_curr.columns:
            dimension = "user_group" # Use normalized column if available
        else:
            return {}

    # Define metric: overdue_rate
    def get_stats(d):
        # Group by dim
        g = d.groupby(dimension).agg(
            vol=("owing_principal", "sum"),
            bad=("overdue_principal", "sum")
        ).reset_index()
        
        total_vol = g["vol"].sum()
        total_bad = g["bad"].sum()
        total_rate = total_bad / total_vol if total_vol > 0 else 0
        
        g["rate"] = g["bad"] / g["vol"]
        g["weight"] = g["vol"] / total_vol if total_vol > 0 else 0
        
        # Return map: key -> {rate, weight}
        stats = {
            r[dimension]: {"rate": r["rate"], "weight": r["weight"]} 
            for _, r in g.iterrows()
        }
        return stats, total_rate

    stats_curr, rate_curr = get_stats(df_curr)
    stats_prev, rate_prev = get_stats(df_prev)
    ...
    all_segs = set(stats_curr.keys()) | set(stats_prev.keys())
    
    for seg in all_segs:
        # Get Curr
        c = stats_curr.get(seg, {"rate": 0, "weight": 0})
        w_c = c["weight"]
        r_c = c["rate"]
        
        # Get Prev
        p = stats_prev.get(seg, {"rate": 0, "weight": 0}) 
        w_p = p["weight"]
        r_p = p["rate"]
        
        # If segment missing in prev, use rate_prev for Mix reference to neutralize noise
        if seg not in stats_prev:
            r_p_mix = rate_prev
            r_p_rate = rate_prev 
        else:
            r_p_mix = r_p
        ...
        # Mix Effect
        mix = (w_c - w_p) * (r_p_mix - rate_prev)
        
        # Rate Effect
        rate_eff = w_c * (r_c - r_p_rate)
        
        if abs(mix) > 0.0001: # Filter tiny noise
            mix_effects.append({"name": str(seg), "val": mix})
        if abs(rate_eff) > 0.0001:
            rate_effects.append({"name": str(seg), "val": rate_eff})
            
    # Sort by impact
    mix_effects.sort(key=lambda x: abs(x["val"]), reverse=True)
    rate_effects.sort(key=lambda x: abs(x["val"]), reverse=True)
    
    return {
        "start_rate": rate_prev,
        "end_rate": rate_curr,
        "delta": rate_curr - rate_prev,
        "mix_effect": mix_effects,
        "rate_effect": rate_effects
    }
...
def compute_aggregated_vintage(df, period='D', limit=30):
    ...
    [v4.5] Renamed entrant_rate to overdue_rate.
    """
    if df is None or df.empty or "due_date" not in df.columns:
        return []
    ...
    for key in sorted_keys:
        ...
        item = {
            "period": key,
            "rows": len(grp),
            "overdue_rate": round(overdue_rate, 4), # Renamed from entrant_rate
            "recovery": {},
            "dpd_rates": {} 
        }
...
def compute_lift_analysis(df, user_type_filter=None):
    ...
    def calc_aggs(sub_df, label):
        ...
        return {
            "period": label,
            "owing": owing,
            "overdue_rate": overdue / owing if owing > 0 else 0, # Renamed from entrant_rate
            "dpd1_rate": rate_d1,
            "dpd5_rate": rate_d5
        }
