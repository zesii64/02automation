...
def generate_report(excel_path):
    ...
    # [Versioning Check]: Force import from v2.2 modules
    try:
        from run_daily_report_v2_2 import (
            compute_vintage_summary,
            compute_due_trend,
            compute_repay_summary,
            compute_process_summary,
            compute_trend_by_period,
            compute_vintage_matrix
        )
        from run_cashloan_report_v2_2 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    except ImportError:
        print("错误: 缺少 v2.2 依赖脚本。请确保 run_daily_report_v2_2.py 和 run_cashloan_report_v2_2.py 在同一目录下。")
        return
    ...
    if df_v is not None:
        try:
            # Note: Functions are already imported from v2.2 module above
            # Overall Trends
