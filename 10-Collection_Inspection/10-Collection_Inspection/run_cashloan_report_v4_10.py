...
def make_natural_month_progress_section(nm_data):
    """生成自然月回收序时进度 ECharts 折线图 + 目标达成汇总表"""
    if not nm_data:
        return ""
    ...
    # 构建 ECharts 数据 JSON — 每个 bucket 一套数据
    charts_data = {}
    for bkt in buckets:
    ...
    for i, bkt in enumerate(buckets):
        ...
        bucket_btns += f'<button class="nm-bucket-btn" data-bucket="{bkt}" style="padding:4px 12px;border-radius:6px;font-size:13px;cursor:pointer;border:1px solid #cbd5e1;background:white;color:#64748b;transition:all 0.2s;margin-right:6px;{active}" onclick="switchNMBucket(\'{bkt}\')">{bkt}</button>'
    ...
    for bkt in buckets:
        s = summary.get(bkt, {})
        if not s:
            summary_rows += f'<tr><td class="px-3 py-2 font-medium">{bkt}</td><td colspan="4" class="text-center text-slate-400">暂无目标数据</td></tr>'
            continue
    ...
    default_bucket = buckets[0] if buckets else ""
    ...
    <div class="card" id="nm-progress-card">
        <div class="flex items-center justify-between mb-4 border-b pb-4">
            ...
            <div class="flex items-center gap-2">{bucket_btns}</div>
        </div>
    ...
    <script>
    (function() {{
        var nmChartsData = {charts_json};
        var nmChart = echarts.init(document.getElementById('nm-chart'));
        var currentBucket = '{default_bucket}';
        
        function renderNMChart(bucket) {{
            var seriesData = nmChartsData[bucket] || [];
            var option = {{
                tooltip: {{
                    trigger: 'axis',
...
def make_due_month_recovery_section(dm_data):
    """生成到期月回收曲线 ECharts 折线图"""
    if not dm_data:
        return ""
    
    import json
    
    buckets = dm_data.get("buckets", [])
    ...
    # 构建每个 (bucket, user_type) 组合的 series 数据
    all_chart_data = {}
    for bkt in buckets:
        for ut in user_types:
            key = f"{bkt}|{ut}"
            combo_data = data.get(key, {})
            series_list = []
            ...
            all_chart_data[key] = series_list
    ...
    for i, bkt in enumerate(buckets):
        active = "background:#eff6ff;color:#2563eb;border-color:#93c5fd;" if i == 0 else ""
        bkt_btns += f'<button class="dm-bucket-btn" data-bucket="{bkt}" style="padding:4px 12px;border-radius:6px;font-size:13px;cursor:pointer;border:1px solid #cbd5e1;background:white;color:#64748b;transition:all 0.2s;margin-right:6px;{active}" onclick="switchDMFilter()">{bkt}</button>'
    
    # 客群筛选按钮
    ut_btns = ""
    for i, ut in enumerate(user_types):
        active = "background:#fef3c7;color:#d97706;border-color:#fcd34d;" if i == 0 else ""
        ut_btns += f'<button class="dm-ut-btn" data-ut="{ut}" style="padding:4px 12px;border-radius:6px;font-size:13px;cursor:pointer;border:1px solid #cbd5e1;background:white;color:#64748b;transition:all 0.2s;margin-right:6px;{active}" onclick="switchDMFilter()">{ut}</button>'
    
    default_bkt = buckets[0] if buckets else "ALL"
    ...
    <div class="card" id="dm-recovery-card">
        ...
        <div class="flex items-center gap-4 mb-4">
            <span class="text-xs text-slate-500 font-medium">Bucket:</span>
            <div class="flex items-center gap-1">{bkt_btns}</div>
            <span class="text-xs text-slate-500 font-medium ml-4">客群:</span>
    ...
    (function() {{
        ...
        function getActiveDMFilter() {{
            var bkt = '{default_bkt}', ut = '{default_ut}';
            document.querySelectorAll('.dm-bucket-btn').forEach(function(btn) {{
                if (btn.style.background.indexOf('eff6ff') >= 0) bkt = btn.dataset.bucket;
            }});
            document.querySelectorAll('.dm-ut-btn').forEach(function(btn) {{
                if (btn.style.background.indexOf('fef3c7') >= 0) ut = btn.dataset.ut;
            }});
            return bkt + '|' + ut;
        }}
        
        function renderDMChart(key) {{
            var seriesData = dmChartsData[key] || [];
            var option = {{
                tooltip: {{
                    trigger: 'axis',
                    formatter: function(params) {{
        ...
        window.switchDMFilter = function() {{
            // Toggle active state on clicked button
            var clicked = event.target;
            if (clicked.classList.contains('dm-bucket-btn')) {{
                document.querySelectorAll('.dm-bucket-btn').forEach(function(btn) {{
                    btn.style.background = 'white'; btn.style.color = '#64748b'; btn.style.borderColor = '#cbd5e1';
                }});
                clicked.style.background = '#eff6ff'; clicked.style.color = '#2563eb'; clicked.style.borderColor = '#93c5fd';
            }} else if (clicked.classList.contains('dm-ut-btn')) {{
                document.querySelectorAll('.dm-ut-btn').forEach(function(btn) {{
                    btn.style.background = 'white'; btn.style.color = '#64748b'; btn.style.borderColor = '#cbd5e1';
                }});
                clicked.style.background = '#fef3c7'; clicked.style.color = '#d97706'; clicked.style.borderColor = '#fcd34d';
            }}
            
            setTimeout(function() {{
                var key = getActiveDMFilter();
                renderDMChart(key);
            }}, 50);
        }};
        
        renderDMChart('{default_bkt}|{default_ut}');
...
<head>
    ...
    <title>CashLoan Inspection Report {date_str} (v4.10)</title>
...
<body class="p-6 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">CashLoan Inspection Report v4.10</h1>
