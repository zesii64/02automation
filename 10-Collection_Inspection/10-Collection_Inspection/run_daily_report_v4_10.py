...
def compute_natural_month_progress(df_daily):
    ...
    输入: DataFrame (natural_month_repay_daily sheet)
      - day, natural_month, case_bucket, data_level, repay_principal, start_owing_principal
    
    输出: dict {
        "buckets": ["S0","S1","S2","M1","M2"],  # 可用的 case_bucket
        "months": [202511, 202512, ...],         # 实际月份（不含 250003）
        "target_month": 250003,                  # 目标月标识
        "data": {
            "S1": {
                202511: [{"day":1, "cum_rate":0.01}, {"day":2, "cum_rate":0.025}, ...],
                202512: [...],
                250003: [...]  # 目标序时
            },
            ...
        },
        "summary": {  # 各 bucket 的目标达成汇总
            "S1": {"target_rate": 0.35, "actual_rate": 0.28, "achieve_pct": 0.80},
            ...
        }
    }
    """
    ...
    # 筛选模块层级 (data_level 中包含 "1." 开头的，通常是最高汇总层级)
    # 目标数据仅存在于此层级
    data_levels = sorted(df['data_level'].dropna().unique())
    module_level = data_levels[0] if data_levels else None
    if module_level:
        df = df[df['data_level'] == module_level]
    
    # 聚合到 (natural_month, case_bucket, day) 粒度
    agg = df.groupby(['natural_month', 'case_bucket', 'day'], as_index=False).agg(
        repay_principal=('repay_principal', 'sum'),
        start_owing_principal=('start_owing_principal', 'sum')
    )
    
    buckets = sorted(agg['case_bucket'].dropna().unique())
    ...
    for bkt in buckets:
        bkt_data = agg[agg['case_bucket'] == bkt]
        result_data[bkt] = {}
        ...
        for month in all_months:
            ...
            # 取该月的 start_owing_principal (同一 bucket 同一月各天应一致，取 day=1 的值)
            base_principal = month_data['start_owing_principal'].iloc[0]
            if base_principal <= 0:
                base_principal = month_data['start_owing_principal'].max()
            
            # 计算累计回收率
            cum_repay = month_data['repay_principal'].cumsum()
            points = []
            for _, row in month_data.iterrows():
                d = int(row['day'])
                cum_val = cum_repay.loc[row.name]
                cum_rate = cum_val / base_principal if base_principal > 0 else 0
                points.append({"day": d, "cum_rate": round(cum_rate, 6)})
            
            result_data[bkt][int(month)] = points
        
        # 计算达成率汇总（用最新实际月 vs 目标）
...
def compute_due_month_recovery(df_due):
    ...
    # 聚合到 (flag_bucket, user_type, due_mth, days_from_duedate) 粒度
    agg = df.groupby(['flag_bucket', 'user_type', 'due_mth', 'days_from_duedate'], as_index=False).agg(
        boc=('overdue_principal_boc', 'sum'),
        eoc=('owing_principal_eoc', 'sum')
    )
    
    buckets = sorted(agg['flag_bucket'].dropna().unique())
    ...
    for bkt in buckets:
        for ut in user_types:
            key = f"{bkt}|{ut}"
            sub = agg[(agg['flag_bucket'] == bkt) & (agg['user_type'] == ut)]
            result_data[key] = {}
