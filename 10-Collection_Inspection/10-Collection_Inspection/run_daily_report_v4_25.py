...
def compute_ops_attribution(df_daily, df_process=None):
    ...
    if not df_l3.empty:
        ...
        # [v4.25] 过滤 Nocall 类非业务组
        df_l3 = df_l3[~df_l3['group_name'].str.contains(r'(?i)nocall|no[_\-\s]?call', na=False)]
    ...
    if not df_l4.empty:
        ...
        # [v4.25] 过滤 Nocall 类非业务组
        df_l4 = df_l4[~df_l4['group_name'].str.contains(r'(?i)nocall|no[_\-\s]?call', na=False)]
...
def compute_smart_diagnostics(df_daily, df_process=None):
    ...
    for ab in sorted(df_l3['agent_bucket'].dropna().unique()):
        ...
        for gn in groups:
            ...
            # [v4.25] 只诊断当月有数据的组（已解散的组不诊断）
            if g_data[g_data['natural_month'] == curr_month].empty:
                continue
            months_list = []
            rates_list = []
            achieves_list = []
