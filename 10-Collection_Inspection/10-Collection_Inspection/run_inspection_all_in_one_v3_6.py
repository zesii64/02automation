...
def generate_report(excel_path):
    ...
    # [Versioning Check]: Force import from v3.6 modules
    try:
        from run_daily_report_v3_6 import (
        ...
        from run_cashloan_report_v3_6 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    except ImportError as e:
        print(f"错误: 缺少 v3.6 依赖脚本 ({e})。")
        return
    ...
    print("  计算归因分析 (Risk Attribution)...")
    attribution_data = {}
    
    if df_v is not None and "due_date" in df_v.columns:
        # Ensure dates
        dates = pd.to_datetime(df_v["due_date"])
        max_date = dates.max()
        if pd.notna(max_date):
            curr_month_start = max_date.replace(day=1)
            days_progress = (max_date - curr_month_start).days + 1
            ...
            # Curr MTD
            df_curr = df_v[(dates >= curr_month_start) & (dates <= max_date)]
            
            # Prev MTD (Same days progress)
            # e.g. If Curr is Jan 1-5, Prev should be Dec 1-5
            prev_target_end = prev_month_start + pd.Timedelta(days=days_progress - 1)
            df_prev = df_v[(dates >= prev_month_start) & (dates <= prev_target_end)]
            
            attribution_data = compute_risk_attribution(df_curr, df_prev)
    ...
    out_path = reports_dir / f"CashLoan_Inspection_Report_{date_str}_v3_6.html"
