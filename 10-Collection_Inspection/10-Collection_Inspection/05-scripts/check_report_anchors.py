"""
生成后 HTML 轻量锚点检查（计划 §②-7）。

用法:
  python check_report_anchors.py [path.html]
  未传路径时仅执行 py 模块自检；CI 可生成后传入产物路径。
"""
...
# 与模板/注数强相关的稳定子串，随大改版增量维护
# 注：generate 末尾 hotfix 可能把 const REAL_DATA 改为 var REAL_DATA
REQUIRED_SUBSTRINGS = [
    "templateContractId",
    "pipelineVersion",
    "report-pipeline:",
