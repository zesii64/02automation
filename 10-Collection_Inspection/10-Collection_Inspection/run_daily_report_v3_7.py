...
def compute_risk_attribution(df_curr, df_prev):
    ...
    Answers: "Which segment caused the risk to rise?"
    """
    ...
    dims = {
        ...
        # "period_no": "Period" # Usually period is fixed in analysis
    }
    ...
    for col, label in dims.items():
        ...
        try:
            ...
            for seg in segments:
                ...
                # Contribution = (Rate_Seg_Curr * Mix_Seg_Curr) - (Rate_Seg_Prev * Mix_Seg_Prev)
                # This sums up to the Total Rate Delta.
    ...
    # 2. Due Date (Batch) Analysis - Top Risky Batches in Current
    # User asked: "Which batch caused it?" -> Implies listing current batches sorted by risk * volume
    if "due_date" in df_curr.columns:
        ...
        for date, sub in g_date:
            ...
            # Contribution to *Current* Rate (not Delta)
            # = Rate * Mix
            contrib = rate * mix 
        ...
        # Sort by Contribution (High Impact Bad Batches)
...
def compute_collection_performance(df):
    ...
    if "agent_bucket" in df.columns:
        g_bucket = df.groupby("agent_bucket").agg({
            "repay_principal": "sum",
        ...
        # 2. For each Bucket, Rank Groups
        out["groups"] = {}
        out["agents"] = {}
        
        for bucket in g_bucket["agent_bucket"].unique():
            sub = df[df["agent_bucket"] == bucket]
            ...
            if "group_name" in sub.columns:
                ...
                g_grp = g_grp.sort_values("repay_rate", ascending=False)
                
                out["groups"][bucket] = {
                    "top": g_grp.head(3).to_dict("records"),
                    "bottom": g_grp.tail(3).to_dict("records"),
                    "variance": g_grp["repay_rate"].std() if len(g_grp) > 1 else 0
                }
