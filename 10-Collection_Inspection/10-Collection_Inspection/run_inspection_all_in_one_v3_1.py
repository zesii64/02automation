# -*- coding: utf-8 -*-
"""
全链路巡检脚本 (All-in-One Inspection Script) v3.2
Feature:
- Split Analysis (Overall/New/Old) for Trends, Lift, Matrix.
- Enhanced MTD Lift (M/M-1/M-2/Y-1) + MoM Lift.
...
def generate_report(excel_path):
    ...
    # [Versioning Check]: Force import from v3.1 modules
    try:
        from run_daily_report_v3_1 import (
        ...
        from run_cashloan_report_v3_1 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    except ImportError as e:
        print(f"错误: 缺少 v3.1 依赖脚本 ({e})。")
        return
    ...
    if df_v is not None and "user_type" in df_v.columns:
        ...
        # [v3.2 Fix] 只要不是 "新客"，都算老客（含 存量老客、新转化老客）
    ...
    out_path = reports_dir / f"CashLoan_Inspection_Report_{date_str}_v3_2.html"
