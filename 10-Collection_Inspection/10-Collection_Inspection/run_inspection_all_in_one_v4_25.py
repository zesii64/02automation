# -*- coding: utf-8 -*-
"""
全链路巡检脚本 (All-in-One Inspection Script) v4.25
Feature:
- v4.25: 时间范围扩至 2024-12 起 (SQL >= '202412')；供线上/943 使用；943 推荐：--mode download 取数后，本机 --mode report 出报告。
- v4.22: [MAJOR] Part 3 运营效能重构 — 可联性迁移到 Part 2, 移除非核心板块,
         新增目标追踪看板/经办排行榜/效率分析/行动项清单; CDN 内联离线支持.
- v4.19: [MAJOR] 归因中心重构 — 增强 Shift-Share (时间窗口+客群+自动摘要), 合并 Risk Attribution.
- v4.18: [FIX] Safe Denominator 逻辑确认 + 页面结构重组 (Part1核心/Part2归因/Part3运营).
- v4.17: [NEW] Shift-Share 结构归因 — DPD 变化 = 结构效应 + 风险效应，瀑布图可视化.
- v4.16: [FIX] Lift 值缺失修复 + 总览层 + Tab翻页 + Bucket排序 + 当月过滤.
- v4.15: [COPY] 基于 v4.14 复制生成，验证输出正确性.
- v4.12: [NEW] 到期月 tooltip 降序排列; 自然月序时增加大小额层级切换.
- v4.11: [FIX] 自然月序时计算逻辑修正 (累计回收/当日累计委案)，到期月回收去掉 bucket 只按客群筛选.
- v4.10: [NEW] 自然月回收序时 + 到期月回收曲线.
- v4.9: Term Monitoring Matrix + Lift Button + Recovery Fallback.
- v4.8: Baseline verified (= v4.2 pure copy).
- v3.6+: Risk Attribution, Amount Heatmap, Collection Performance, Contactability.
"""
...
WHERE 1 = 1
  ...
  AND TO_CHAR(dt_biz, 'yyyyMM') >= '202412'
...
# ─── [v4.12 FIX] 自然月回收序时 — 日粒度（含 data_level / 目标行 250003） ───
# [v4.12] 去掉 case_bucket 硬编码过滤 (v4.10 的 IN ('S0','S1',...) 会漏掉 1.5.大小模块层级)
#         保留全部 data_level，以支持未来 group/agent 级别下钻
SQL_REPAY_DAILY = """
-- 自然月回收日粒度，用于序时进度曲线 + 目标达成
-- 包含所有 data_level (1.模块/1.5.大小模块/2.5内外/3.公司/4.组别/5.经办)
-- 目标行: natural_month = '250003'
SELECT day
     ...
     , SUM(start_owing_principal)    AS start_owing_principal
FROM phl_anls.tmp_maoruochen_phl_repay_natural_day_daily
WHERE TO_CHAR(dt_biz, 'yyyyMM') >= '202412'
GROUP BY day
...
# ─── [v4.10 NEW] 到期月回收曲线 — 按 Bucket / 客群 ───
SQL_DUE_MONTH_REPAY = """
-- 到期月回收曲线：距到期日天数 × 到期月 × 客群 × Bucket
-- 回收率 = 1 - SUM(owing_principal_eoc) / SUM(overdue_principal_boc)
SELECT days_from_duedate
     , due_mth
     , user_type
     , flag_bucket
...
SELECT TO_CHAR(TO_DATE(dt), 'yyyymm') AS natural_month
     ...
     -- [v4.25] cover_rate 改为真实单日触达率：每个经办每天拨打的号码数 / 持有案件数，再取日均
     -- 旧口径 (own_uncomm_case_cnt) 是月内累计，一个月碰一次就标"已覆盖"，不具区分度
     , AVG(CASE WHEN owing_case_cnt > 0 THEN LEAST(CAST(call_mobile_cnt AS DOUBLE) / owing_case_cnt, 1.0) END) AS cover_rate
     ...
     -- [v4.25] 新增：组当月活跃天数 & 活跃人数，用于过滤月初取消的组
     , COUNT(DISTINCT dt) AS active_days
     , COUNT(DISTINCT CASE WHEN last_call_hour - first_call_hour > 0 THEN owner_name END) AS agent_count
...
def _normalize_dtypes(df):
    """[v4.25] 将 ODPS DataFrame 的类型对齐到「从 Excel 读取」后的效果。
    方法：内存 CSV 序列化/反序列化 + 显式解析日期列。"""
    import io
    buf = io.StringIO()
    df.to_csv(buf, index=False)
    buf.seek(0)
    # 找到列名含 'date' 的列，显式解析为 datetime（如 due_date）
    date_cols = [c for c in df.columns if 'date' in c.lower()]
    return pd.read_csv(buf, parse_dates=date_cols if date_cols else False)


def fetch_data_to_memory():
    """[v4.25] 从 ODPS 取数到内存，返回 dict {sheet_name: DataFrame}"""
    o = get_odps_entry()
    if not o:
    ...
    tasks = [
        ...
        ("natural_month_repay_daily", SQL_REPAY_DAILY),
        ("due_month_repay", SQL_DUE_MONTH_REPAY),
    ]

    print("开始从 ODPS 取数 (内存模式) ...")
    data = {}
...
def save_data_to_excel(data, output_file="collection_inspection_data_local.xlsx"):
    """[v4.25] 把内存中的 dict {sheet_name: DataFrame} 写入 Excel"""
    print(f"正在保存数据至 {output_file} ...")
    try:
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            for sheet_name, df in data.items():
                ...
                if len(df) > MAX_ROWS:
                    ...
                    print(f"  {sheet_name}: {len(df)} 行，拆分为 {parts} 个 Sheet...")
        ...
        print(f"保存 Excel 异常: {e}")
        import traceback
        traceback.print_exc()


def download_data(output_file="collection_inspection_data_local.xlsx"):
    """在 ODPS 环境执行 SQL 并保存为 Excel（兼容旧 --mode download）"""
    data = fetch_data_to_memory()
    if data:
        save_data_to_excel(data, output_file)


def generate_report(excel_path=None, memory_data=None):
    """生成 HTML 报表。
    数据来源二选一：
      - excel_path: 从 Excel 文件读取（兼容 --mode report / --mode all）
      - memory_data: 直接传入 {sheet_name: DataFrame}（--mode online 内存直出）
    """
    # 尝试导入 run_daily_report 中的计算逻辑
    try:
        from run_daily_report_v4_25 import (
            ...
            compute_term_monitoring_matrix,
            compute_natural_month_progress,
            compute_due_month_recovery,
            compute_shift_share,
            compute_ops_attribution,
            compute_smart_diagnostics
        )
        from run_cashloan_report_v4_25 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    except ImportError as e:
        print(f"错误: 缺少 v4.2 依赖脚本 ({e})。")
        return

    # 1. 读取数据 — 内存优先，否则从 Excel
    if memory_data:
        print("使用内存数据生成报告（跳过 Excel 读写）...")
        df_v = memory_data.get("vintage_risk")
        df_repay = memory_data.get("natural_month_repay")
        repay_name = "natural_month_repay"
        df_process = memory_data.get("process_data")
        df_repay_daily = memory_data.get("natural_month_repay_daily")
        df_due_month = memory_data.get("due_month_repay")
    else:
        ...
        df_repay_daily = read_sheet(excel_path, "natural_month_repay_daily")
        df_due_month = read_sheet(excel_path, "due_month_repay")

    print(f"  natural_month_repay_daily: {len(df_repay_daily) if df_repay_daily is not None else 0} rows")
    print(f"  due_month_repay: {len(df_due_month) if df_due_month is not None else 0} rows")
    ...
    print("  计算自然月回收序时 (Natural Month Progress)...")
    nm_progress_data = compute_natural_month_progress(df_repay_daily)
    
    print("  计算到期月回收曲线 (Due Month Recovery Curve)...")
    dm_recovery_data = compute_due_month_recovery(df_due_month)

    # [v4.17 NEW] Shift-Share 结构归因分析
    print("  计算 Shift-Share 结构归因 (Structure vs Risk Attribution)...")
    shift_share_data = compute_shift_share(df_v)

    # [v4.21 NEW] 运营归因 Treemap
    print("  计算运营归因 (Ops Attribution Treemap)...")
    ops_attr_data = compute_ops_attribution(df_repay_daily, df_process)

    # [v4.23 NEW] 智能诊断
    print("  计算智能诊断 (Smart Diagnostics)...")
    smart_diag_data = compute_smart_diagnostics(df_repay_daily, df_process)

    # [v4.23 NEW] Process Detail — 按 (owner_bucket, owner_group) 聚合的过程指标
    print("  准备过程指标明细 (Process Detail by Group)...")
    process_detail = []
    if df_process is not None and not df_process.empty:
        proc_cols = ["natural_month", "owner_bucket", "owner_group",
                     "call_times_avg", "cover_rate", "case_connect_rate",
                     "call_connect_rate", "call_billhr_avg", "caseload",
                     "raw_call_times", "raw_call_connect_times",
                     "raw_owing_case_cnt", "raw_uncomm_case_cnt"]
        available_cols = [c for c in proc_cols if c in df_process.columns]
        for _, row in df_process[available_cols].iterrows():
            process_detail.append({c: row[c] for c in available_cols})
    ...
    html = build_cashloan_html(
        ...
        nm_progress_data=nm_progress_data,
        dm_recovery_data=dm_recovery_data,
        # v4.17 Shift-Share
        shift_share_data=shift_share_data,
        # v4.21 Ops Attribution
        ops_attr_data=ops_attr_data,
        # v4.23 Process Detail
        process_detail=process_detail,
        # v4.23 Smart Diagnostics
        smart_diag_data=smart_diag_data
    )
    
    # 4. 写入文件
    reports_dir = Path(__file__).parent / "reports"
    reports_dir.mkdir(exist_ok=True)
    out_path = reports_dir / f"CashLoan_Inspection_Report_{date_str}_v4_25.html"
...
def main():
    ...
    parser.add_argument("--mode", choices=["download", "report", "all", "online"], default="report", 
                        help="运行模式: download(仅取数), report(仅报表), all(取数→存Excel→读Excel→报表), "
                             "online(取数→内存直出报告→再存Excel，943推荐)")
    ...
    # ── [v4.25 NEW] online 模式：内存直出报告，再存 Excel 留档 ──
    if args.mode == "online":
        data = fetch_data_to_memory()
        if not data:
            print("取数失败，终止。")
            return
        # 1) 先出报告（快，内存直传，无 Excel I/O）
        generate_report(memory_data=data)
        # 2) 再存 Excel 留档（慢，但报告已经好了）
        save_data_to_excel(data, args.excel)
        return
    ...
    if args.mode in ["report", "all"]:
        ...
        generate_report(excel_path=excel_path)
