# -*- coding: utf-8 -*-
"""
全链路巡检脚本 (All-in-One Inspection Script) v4.5
Feature:
- v4.5: Waterfall Attribution (Risk Mix/Rate Effect); standardized 'overdue_rate'.
- v4.4: Vintage Matrix Lift Toggle; Term Matrix Dynamic Cols/Tabs/Metrics.
- v4.3: Term Monitoring Matrix (到期月×MOB1~4, 新老客/1期3期6期筛选); period_seq in SQL.
- v4.3: Risk Attribution 放款账龄用 period_no (MOB).
- v4.2: 当前月份过滤 25 开头 (目标月); 报告标题 v4.2.
- v4.1: Recovery Attribution; v4.0: Contactability.
- v3.6: Risk Attribution, Loan Cnt, Signature.
- v3.5: Amount Heatmap, Naming, Arrow 0.1%.
- Split (Overall/New/Old); MTD/MoM Lift; Vintage Matrix; Loan Count in SQL.
"""
...
def generate_vintage_sql():
    ...
    # 1. 维度定义 (Dimensions) [v4.3] 增加 period_seq 支持 1期/3期/6期 产品筛选
    dims = [
...
def generate_report(excel_path):
    ...
    # [Versioning Check]: Import from v4.5 modules
    try:
        from run_daily_report_v4_5 import (
            ...
            compute_waterfall_data  # [v4.5 New]
        )
        from run_cashloan_report_v4_5 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    except ImportError as e:
        print(f"错误: 缺少 v4.5 依赖脚本 ({e})。")
        return
    ...
    # [v3.2 New] Multi-dim Vintage Matrix
    matrix_daily = []
    matrix_weekly = []
    matrix_monthly = []
    ...
    # [v3.5 New] Amount Pivot
    print("  计算金额段热力图 (Amount Pivot)...")
    amount_pivot_data = {}
    if df_v is not None:
        amount_pivot_data = compute_amount_pivot(df_v)

    # [v3.6 New] Risk Attribution (MoM)
    print("  计算归因分析 (Risk Attribution - Segmented)...")
    attribution_data = {}
    
    # [v4.5 New] Waterfall Analysis Calculation
    print("  计算归因瀑布图 (Waterfall)...")
    waterfall_data = {}
    ...
    if df_v is not None and "due_date" in df_v.columns:
        ...
        def get_mtd_slices(df_target):
            ...
            # Re-derive global context (Align with Global Max to be safe)
        ...
        # [v4.5] Compute Waterfall Data
        # We need to pass the 'user_group' column if we want New/Old grouping for Mix Effect
        # Ensure user_group exists if we use compute_term_monitoring_matrix mapping logic,
        # but here we can just do simple mapping on the fly or reuse.
        # compute_waterfall_data handles raw user_type if needed, or we can map it first.
        # Let's map it for better readability in the chart.
        if dc is not None and not dc.empty:
            ...
            if "user_type" in dc.columns:
                dc = dc.copy()
                dc["user_group"] = dc["user_type"].apply(map_user)
            if dp is not None and not dp.empty and "user_type" in dp.columns:
                dp = dp.copy()
                dp["user_group"] = dp["user_type"].apply(map_user)
                
            # Compute Waterfall using 'user_group' dimension
            waterfall_data = compute_waterfall_data(dc, dp, dimension="user_group")
    ...
    # [v4.3] Term Monitoring Matrix (到期月×MOB1~4)
    ...
    out_path = reports_dir / f"CashLoan_Inspection_Report_{date_str}_v4_5.html"
