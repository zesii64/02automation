...
def make_natural_month_progress_section(nm_data):
    """[v4.12] 生成自然月回收序时进度 ECharts 折线图 + 目标达成汇总表 + 层级切换"""
    if not nm_data:
        return ""
    ...
    # [v4.12] 支持多层级
    levels = nm_data.get("levels", [])
    level_data = nm_data.get("level_data", {})
    
    # 如果没有 levels 信息 (向后兼容), 构造单层级
    if not levels:
        levels = ["模块级"]
        level_data = {"模块级": {
            "buckets": nm_data.get("buckets", []),
            "months": nm_data.get("months", []),
            "target_month": nm_data.get("target_month", 250003),
            "data": nm_data.get("data", {}),
            "summary": nm_data.get("summary", {})
        }}
    
    # 月份颜色映射 (最新月最醒目)
    month_colors = ['#94a3b8', '#64748b', '#475569', '#3b82f6', '#2563eb', '#1d4ed8', '#1e40af']
    target_color = '#ef4444'  # 红色虚线
    
    # ── 构建所有层级的 ECharts 数据 JSON ──
    all_levels_charts = {}   # { level_name: { bucket: [series...] } }
    all_levels_meta = {}     # { level_name: { buckets:[], months:[], target_month:int } }
    all_levels_summary = {}  # { level_name: { bucket: {summary} } }
    
    for lvl_name in levels:
        lvl = level_data.get(lvl_name, {})
        buckets = lvl.get("buckets", [])
        months = lvl.get("months", [])
        target_month = lvl.get("target_month", 250003)
        data = lvl.get("data", {})
        summary = lvl.get("summary", {})
        
        if not buckets or not data:
            continue
        
        charts_data = {}
        for bkt in buckets:
            bkt_data = data.get(bkt, {})
            series_list = []
            
            for i, m in enumerate(months):
                if m not in bkt_data:
                    continue
                points = bkt_data[m]
                color_idx = min(i, len(month_colors) - 1)
                ...
                series_list.append({
                    ...
                    "smooth": True,
                    "symbol": "none",
                    "lineStyle": {"width": 3 if is_latest else 1.5, "type": "solid"},
                    "itemStyle": {"color": month_colors[color_idx]},
                    "emphasis": {"lineStyle": {"width": 3}},
                    "data": [[p["day"], round(p["cum_rate"] * 100, 2)] for p in points]
                })
            
            # 目标线 (虚线)
            if target_month in bkt_data:
                target_points = bkt_data[target_month]
                series_list.append({
                    "name": "Target",
                    "type": "line",
                    "smooth": True,
                    "symbol": "none",
                    "lineStyle": {"width": 2.5, "type": "dashed", "color": target_color},
                    "itemStyle": {"color": target_color},
                    "data": [[p["day"], round(p["cum_rate"] * 100, 2)] for p in target_points]
                })
            
            charts_data[bkt] = series_list
        
        all_levels_charts[lvl_name] = charts_data
        all_levels_meta[lvl_name] = {"buckets": buckets, "months": months, "target_month": target_month}
        all_levels_summary[lvl_name] = summary
    
    if not all_levels_charts:
        return ""
    
    # JSON 序列化
    charts_json = json.dumps(all_levels_charts, ensure_ascii=False)
    meta_json = json.dumps(all_levels_meta, ensure_ascii=False)
    summary_json = json.dumps(all_levels_summary, ensure_ascii=False)
    
    # ── 构建层级切换按钮 ──
    level_btns = ""
    for i, lvl_name in enumerate(levels):
        if lvl_name not in all_levels_charts:
            continue
        active = "background:#10b981;color:white;border-color:#10b981;" if i == 0 else ""
        level_btns += f'<button class="nm-level-btn" data-level="{lvl_name}" style="padding:4px 14px;border-radius:6px;font-size:13px;cursor:pointer;border:1px solid #cbd5e1;background:white;color:#64748b;transition:all 0.2s;margin-right:6px;font-weight:600;{active}" onclick="switchNMLevel(this)">{lvl_name}</button>'
    
    # ── 目标达成汇总表 (初始为第一层级) ──
    first_level = levels[0] if levels else ""
    first_buckets = all_levels_meta.get(first_level, {}).get("buckets", [])
    first_summary = all_levels_summary.get(first_level, {})
    
    summary_rows = ""
    for bkt in first_buckets:
        s = first_summary.get(bkt, {})
        if not s:
            summary_rows += f'<tr><td class="px-3 py-2 font-medium">{bkt}</td><td colspan="5" class="text-center text-slate-400">暂无目标数据</td></tr>'
            continue
        
        target_r = s.get("target_rate_at_day", 0)
        actual_r = s.get("actual_rate", 0)
        achieve = s.get("achieve_pct", 0)
        actual_day = s.get("actual_day", 0)
        latest_m = s.get("latest_month", 0)
        
        bar_color = "#22c55e" if achieve >= 1.0 else ("#f59e0b" if achieve >= 0.9 else "#ef4444")
        bar_width = min(achieve * 100, 100)
        
        summary_rows += f'''<tr class="border-t border-slate-100">
            <td class="px-3 py-2 font-medium text-slate-700">{bkt}</td>
            <td class="px-3 py-2 text-right">{target_r:.2%}</td>
            <td class="px-3 py-2 text-right font-semibold">{actual_r:.2%}</td>
            <td class="px-3 py-2 text-right font-bold" style="color:{bar_color}">{achieve:.1%}</td>
            <td class="px-3 py-2">
                <div style="background:#e2e8f0;border-radius:4px;height:8px;width:100px;position:relative;">
                    <div style="background:{bar_color};border-radius:4px;height:8px;width:{bar_width:.0f}px;"></div>
                </div>
            </td>
            <td class="px-3 py-2 text-xs text-slate-400">Day {actual_day} / {latest_m}</td>
        </tr>'''
    
    default_level = first_level
    default_bucket = first_buckets[0] if first_buckets else ""
    ...
    <div class="card" id="nm-progress-card">
        ...
        <!-- [v4.12] 层级切换 + Bucket 筛选 -->
        <div class="flex items-center gap-4 mb-3">
            <span class="text-xs text-slate-500 font-medium">层级:</span>
            <div class="flex items-center gap-1">{level_btns}</div>
        </div>
        <div class="flex items-center gap-4 mb-4">
            <span class="text-xs text-slate-500 font-medium">模块:</span>
            <div class="flex items-center gap-1 flex-wrap" id="nm-bucket-btns-container"></div>
        </div>
        
        <div id="nm-chart" style="width:100%;height:400px;"></div>
        
        <!-- 目标达成汇总 -->
        <div class="mt-6">
            <h3 class="text-sm font-semibold text-slate-600 mb-3">目标达成汇总 (Target Achievement)</h3>
            <table class="w-full text-sm">
                ...
                <tbody id="nm-summary-tbody">{summary_rows}</tbody>
            </table>
        </div>
    </div>
    ...
    <script>
    (function() {{
        var nmAllChartsData = {charts_json};
        var nmMeta = {meta_json};
        var nmSummary = {summary_json};
        var nmChart = echarts.init(document.getElementById('nm-chart'));
        var currentNMLevel = '{default_level}';
        var currentNMBucket = '{default_bucket}';
        
        function renderNMChart(bucket) {{
            var levelData = nmAllChartsData[currentNMLevel] || {{}};
            var seriesData = levelData[bucket] || [];
            var option = {{
                tooltip: {{
                    trigger: 'axis',
                ...
                yAxis: {{ type: 'value', name: 'Cum Recovery %', axisLabel: {{ formatter: '{{value}}%', fontSize: 11 }} }},
                series: seriesData
            }};
            nmChart.setOption(option, true);
        }}
        
        function buildBucketBtns(level) {{
            var container = document.getElementById('nm-bucket-btns-container');
            container.innerHTML = '';
            var meta = nmMeta[level] || {{}};
            var buckets = meta.buckets || [];
            
            buckets.forEach(function(bkt, i) {{
                var btn = document.createElement('button');
                btn.className = 'nm-bucket-btn';
                btn.dataset.bucket = bkt;
                btn.textContent = bkt;
                ...
                btn.onclick = function() {{ switchNMBucket(bkt); }};
        ...
        function updateSummaryTable(level) {{
            var tbody = document.getElementById('nm-summary-tbody');
            var summaryData = nmSummary[level] || {{}};
            var meta = nmMeta[level] || {{}};
            var buckets = meta.buckets || [];
            var html = '';
            
            buckets.forEach(function(bkt) {{
                var s = summaryData[bkt];
                if (!s) {{
                    html += '<tr><td class="px-3 py-2 font-medium">' + bkt + '</td><td colspan="5" class="text-center text-slate-400">暂无目标数据</td></tr>';
                    return;
                }}
                var targetR = s.target_rate_at_day || 0;
                var actualR = s.actual_rate || 0;
                var achieve = s.achieve_pct || 0;
                var actualDay = s.actual_day || 0;
                var latestM = s.latest_month || 0;
                
                var barColor = achieve >= 1.0 ? '#22c55e' : (achieve >= 0.9 ? '#f59e0b' : '#ef4444');
                var barWidth = Math.min(achieve * 100, 100);
                
                html += '<tr class="border-t border-slate-100">' +
                    '<td class="px-3 py-2 font-medium text-slate-700">' + bkt + '</td>' +
                    '<td class="px-3 py-2 text-right">' + (targetR * 100).toFixed(2) + '%</td>' +
                    '<td class="px-3 py-2 text-right font-semibold">' + (actualR * 100).toFixed(2) + '%</td>' +
                    '<td class="px-3 py-2 text-right font-bold" style="color:' + barColor + '">' + (achieve * 100).toFixed(1) + '%</td>' +
                    '<td class="px-3 py-2"><div style="background:#e2e8f0;border-radius:4px;height:8px;width:100px;position:relative;"><div style="background:' + barColor + ';border-radius:4px;height:8px;width:' + barWidth.toFixed(0) + 'px;"></div></div></td>' +
                    '<td class="px-3 py-2 text-xs text-slate-400">Day ' + actualDay + ' / ' + latestM + '</td>' +
                    '</tr>';
            }});
        ...
        window.switchNMBucket = function(bucket) {{
            currentNMBucket = bucket;
            document.querySelectorAll('.nm-bucket-btn').forEach(function(btn) {{
            ...
            renderNMChart(bucket);
        }};
        
        window.switchNMLevel = function(btn) {{
            var level = btn.dataset.level;
            currentNMLevel = level;
            
            // 重置层级按钮样式
            document.querySelectorAll('.nm-level-btn').forEach(function(b) {{
            ...
            // 重建 bucket 按钮
            var firstBucket = buildBucketBtns(level);
            currentNMBucket = firstBucket;
            
            // 更新汇总表
            updateSummaryTable(level);
            
            // 渲染图表
            renderNMChart(firstBucket);
        }};
        
        // 初始化
        buildBucketBtns(currentNMLevel);
        renderNMChart(currentNMBucket);
