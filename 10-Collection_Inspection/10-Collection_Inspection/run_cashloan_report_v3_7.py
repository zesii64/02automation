...
def make_collection_performance_section(perf_data):
    ...
    for b in buckets:
        ...
        rows_b.append(f"<tr><td class='p-2 font-medium'>{name}</td><td class='text-right p-2'>{int(vol):,}</td><td class='text-right p-2 font-bold text-green-600'>{rr:.2%}</td></tr>")
    
    html_bucket = f"""
    <div class="mb-6">
        <h3 class="text-sm font-bold text-gray-700 mb-2">整体阶段回收 (Bucket Overview)</h3>
        <table class="w-full text-sm border rounded">
            <thead class="bg-gray-50"><tr><th class="text-left p-2">Bucket</th><th class="text-right p-2">Owing Vol</th><th class="text-right p-2">Repay Rate</th></tr></thead>
    ...
    for bucket, data in groups.items():
        ...
        def _render_list(lst, title_color):
            if not lst: return "-"
            res = []
            for item in lst:
                res.append(f"{item['group_name']} ({item['repay_rate']:.1%})")
            return "<br>".join(res)
            
        html_groups += f"""
        <div class="mb-4 border rounded p-4">
            <h4 class="font-bold text-sm text-gray-800 mb-2">{bucket} - Group Rankings</h4>
            <div class="grid grid-cols-2 gap-4 text-xs">
                <div>
                    <div class="text-green-600 font-bold mb-1">Top 3 Groups</div>
                    {_render_list(top, 'green')}
                </div>
                <div>
                    <div class="text-red-600 font-bold mb-1">Bottom 3 Groups</div>
                    {_render_list(bottom, 'red')}
                </div>
            </div>
        </div>
        """
...
<head>
    ...
    <title>CashLoan Inspection Report {date_str} (v3.7)</title>
...
<body class="p-6 max-w-[1400px] mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">CashLoan Inspection Report v3.7</h1>
