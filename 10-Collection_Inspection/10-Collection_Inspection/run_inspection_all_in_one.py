...
使用方式：
    python run_inspection_all_in_one.py --mode download  (在 943 上跑)
    python run_inspection_all_in_one.py --mode report    (在本地跑)
    python run_inspection_all_in_one.py --mode all       (如果本地能连库)
...
def generate_report(excel_path):
    ...
    try:
        ...
        from run_cashloan_report import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    except ImportError:
        print("错误: 缺少依赖脚本 run_daily_report.py 或 run_cashloan_report.py。")
        print("请确保这些文件在同一目录下。")
        return
    ...
    # 计算多维趋势 (Overall, New, Old)
    trend_d, trend_w, trend_m = [], [], []
    trend_d_new, trend_w_new, trend_m_new = [], [], []
    trend_d_old, trend_w_old, trend_m_old = [], [], []
    
    # 计算 Vintage 矩阵 (Overall, New, Old)
    matrix_all, matrix_new, matrix_old = [], [], []
    
    if df_v is not None:
        try:
            from run_daily_report import compute_trend_by_period, compute_vintage_matrix
            # Overall Trends
            ...
            if "user_type" in df_v.columns:
                ...
                # Matrices
                matrix_all = compute_vintage_matrix(df_v, limit=21)
                matrix_new = compute_vintage_matrix(df_new, limit=21)
                matrix_old = compute_vintage_matrix(df_old, limit=21)
            else:
                matrix_all = compute_vintage_matrix(df_v, limit=21)
                
        except ImportError:
            trend_d = compute_due_trend(df_v, max_days=21)
    ...
    # Process summary not returned as tuple in imported function? check run_daily_report
    # run_daily_report.compute_process_summary returns dict (anomalies inside?)
    # Let's check: actually run_daily_report's compute_process_summary might not return anomalies separately
    # Checking source... compute_process_summary returns dict.
    ...
    html = build_cashloan_html(
        ...
        matrix_all=matrix_all, matrix_new=matrix_new, matrix_old=matrix_old
    )
