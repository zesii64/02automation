...
def make_shift_share_section(ss_data):
    ...
    # 默认客群标签（避免 f-string 内反斜杠 \u 导致 SyntaxError）
    default_ut_label = user_types[0] if user_types else "\u5168\u90e8"
    ...
    (function() {{
        ...
        var state = {{ ut: '{default_ut_label}', mk: '{default_mk}', dk: '{default_dk}' }};
...
def _build_group_diagnostics(nm_data, process_detail, perf_data):
    ...
    [v4.25] 增加三层过滤：
      A. 过滤 Nocall / Target 等非业务组名
      B. 交叉校验：只保留当月 process_data 中也存在的组
      C. cover_rate 一致性：raw_call_times=0 时 cover_rate 置 None
    """
    import re as _re
    ...
    # ── [v4.25] 非业务组名过滤规则 ──
    _EXCLUDE_PATTERNS = _re.compile(
        r'(?i)(nocall|no[_\-\s]?call|target)', _re.IGNORECASE
    )

    def _is_valid_group(gname):
        """判断组名是否为有效业务组"""
        if not gname or not str(gname).strip():
            return False
        return not _EXCLUDE_PATTERNS.search(str(gname))

    level_data = nm_data.get("level_data", {})
    group_level = level_data.get("组级", {})
    agent_level = level_data.get("经办级", {})

    # ── [v4.25] 构建当月 process_data 白名单 ──
    proc_current_groups = set()  # (bucket_prefix, group_name)
    proc_lookup = {}
    if process_detail:
        # 找到 process_detail 中最新月份
        all_proc_months = set()
        for row in process_detail:
            nm = row.get("natural_month")
            if nm is not None:
                try:
                    all_proc_months.add(int(float(nm)))
                except (ValueError, TypeError):
                    pass
        latest_proc_month = max(all_proc_months) if all_proc_months else None

        for row in process_detail:
            ob = str(row.get("owner_bucket", "")).strip()
            og = str(row.get("owner_group", "")).strip()
            if not ob or not og:
                continue
            pk = (ob, og)
            # 只取最新月数据做白名单 & lookup
            row_month = None
            try:
                row_month = int(float(row.get("natural_month", 0)))
            except (ValueError, TypeError):
                pass
            if latest_proc_month and row_month == latest_proc_month:
                proc_current_groups.add(pk)
    ...
    for parent, pdata in group_level.get("by_parent", {}).items():
        for gname, sdata in pdata.get("summary", {}).items():
            # [v4.25] 过滤 Nocall / Target 等非业务组
            if not _is_valid_group(gname):
                continue
            key = (parent, gname)
            diag[key] = {
                "parent": parent,
    ...
    # 3. 从 process_detail 获取过程指标 (使用已构建的 proc_lookup)
    if proc_lookup:
                ...
                for metric in ["call_times_avg", "cover_rate", "call_billhr_avg", "case_connect_rate", "caseload"]:
                    ...
                    if val is not None:
                        try:
                            ...
                            pass
                # [v4.25] 一致性校验：raw_call_times=0 时，cover_rate 不可信，置 None
                raw_calls = 0
                try:
                    raw_calls = float(row.get("raw_call_times", 0) or 0)
                except (ValueError, TypeError):
                    pass
                if raw_calls <= 0:
                    d["cover_rate"] = None
                    d["case_connect_rate"] = None

    # ── [v4.25] 交叉校验：只保留当月 process_data 中也存在的组 ──
    # 同时移除 agent_count=0 的空组 & 活跃天数过少的组（月初即取消）
    # 活跃天数用相对比例判断，避免月初 1~2 号跑数时误删所有组
    _ACTIVE_DAYS_RATIO = 0.3  # 活跃天数 < 同月最大值的 30% 视为异常组
    if proc_current_groups:
        # 先找当月所有组中最大的活跃天数
        max_active_days = 0
        for pk in proc_current_groups:
            row = proc_lookup.get(pk)
            if row:
                try:
                    ad = int(float(row.get("active_days", 0) or 0))
                except (ValueError, TypeError):
                    ad = 0
                if ad > max_active_days:
                    max_active_days = ad
        ...
        for key, d in diag.items():
            ...
            gname = key[1]
            pk = (bp, gname)
            if pk not in proc_current_groups:
                keys_to_remove.append(key)
            elif d["agent_count"] == 0:
                keys_to_remove.append(key)
            elif max_active_days >= 3:
                # 月进度 >= 3 天时才启用活跃天数过滤（避免 1~2 号误删）
                row = proc_lookup.get(pk)
                if row:
                    try:
                        active_days = int(float(row.get("active_days", 0) or 0))
                    except (ValueError, TypeError):
                        active_days = 0
                    if active_days < max_active_days * _ACTIVE_DAYS_RATIO:
                        keys_to_remove.append(key)
