# -*- coding: utf-8 -*-
"""
全链路巡检脚本 (All-in-One Inspection Script) v4.10
Feature:
- v4.10: [NEW] 自然月回收序时 (Natural Month Recovery Progress) — 日粒度取数 + data_level.
- v4.10: [NEW] 到期月回收曲线 (Due Month Recovery Curve) — 按 Bucket / 客群 筛选.
...
# ─── [v4.10 NEW] 自然月回收序时 — 日粒度（含 data_level / 目标行 250003） ───
SQL_REPAY_DAILY = """
...
def generate_report(excel_path):
    ...
    try:
        from run_daily_report_v4_10 import (
            ...
            compute_natural_month_progress,   # [v4.10 NEW]
            compute_due_month_recovery        # [v4.10 NEW]
        )
        from run_cashloan_report_v4_10 import build_cashloan_html, read_sheet_maybe_chunked, read_sheet
    ...
    out_path = reports_dir / f"CashLoan_Inspection_Report_{date_str}_v4_10.html"
