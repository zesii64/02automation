...
def compute_aggregated_vintage(df, period='D', limit=30):
    """
    [v3 New] Multi-dimension Vintage Matrix (Daily/Weekly/Monthly).
    Features:
    1. Safe Denominator: When aggregating (e.g. Week), only sum 'overdue' for days that are mature for that specific D_k.
    2. Lag 1 Mapping: Report D(k) uses Source d(k+1).
    """
    if df is None or df.empty or "due_date" not in df.columns:
        return []
    ...
    # Pre-calculate maturity for each row (day)
    # limit analysis to relevant columns
    today = pd.Timestamp.now().normalize()
    
    # We need to aggregate by period_key.
    # But for Recovery Rates, we can't just sum(d_k)/sum(overdue) because overdue includes immature days.
    # Algorithm:
    # 1. Group by [period_key].
    # 2. Inside each group (which contains multiple days), for each D_k metric:
    #    Numerator = Sum(d(k+1)_principal) where (Today - due_date >= k+1)
    #    Denominator = Sum(overdue_principal) where (Today - due_date >= k+1)
    #    Rate = 1 - Num/Denom.
    
    # To do this efficiently in Pandas:
    # We can expand the dataset or iterate groups. Iterating groups is safer for complex logic.
    ...
    for key in sorted_keys:
        ...
        # 1. Entrant Rate (Overdue / Owing) - Base logic
        # Usually Entrant Rate is finalized at T+1.
        # Check maturity: if T+1 is not reached, overdue is not ready?
        # Assuming source data filters T+1.
        owing = grp["owing_principal"].sum()
        overdue = grp["overdue_principal"].sum()
        ...
        # 2. Recovery D1..D30
        for k in range(1, 31): # Report D1..D30
            # Source col: d(k+1)
            src_col = f"d{k+1}_principal"
            
            if src_col not in grp.columns:
                item["recovery"][f"D{k}"] = None
                continue
                
            # Filter mature rows within this period group
            # Condition: (Today - DueDate) >= k + 1
            # Optimization: Pre-calc lag
            ...
            if not mask.any():
                ...
                # Sum only mature rows
                mature_overdue = grp.loc[mask, "overdue_principal"].sum()
                mature_bal = grp.loc[mask, src_col].sum()
                
                # [Fix]: D1 Proxy Logic (if D1 is 0/Overdue, use D2)
                # Note: For aggregated view, this is complex. Simplified: Use raw sum.
                # If mature_overdue > 0
                if mature_overdue > 0:
...
def compute_lift_analysis(df):
    """
    [v3 New] MTD (Month-to-Date) Comparison / Lift Analysis.
    Compares:
    1. Current Month (MTD)
    ...
    # Days passed in current month (inclusive of start, up to yesterday usually, but let's say up to today)
    # Actually, vintage is by Due Date. If today is Feb 4, we have Feb 1, 2, 3...
    # Let's take all available data for current month.
    ...
    # 2. Last Month MTD
    # Last month start
    last_month_start = (curr_month_start - pd.Timedelta(days=1)).replace(day=1)
    last_month_end_mtd = last_month_start + pd.Timedelta(days=days_progress - 1)
    
    df_prev = df[(df["due_date"] >= last_month_start) & (df["due_date"] <= last_month_end_mtd)]
    ...
    def calc_aggs(sub_df, label):
        ...
        # DPD1 (Report D1) -> Source d2. Mature rows only?
        # For MTD comparison, we compare "State at T+X".
        # If Current Month includes immature DPD1, we should exclude them OR include them (as 0).
        # Standard Lift compares "Available Data".
        # Using Safe Denominator logic:
        
        # DPD1 (d2): T+2
        mask_d1 = (today - sub_df["due_date"]).dt.days >= 2
        ov_d1 = sub_df.loc[mask_d1, "overdue_principal"].sum()
        bal_d1 = sub_df.loc[mask_d1, "d2_principal"].sum() if "d2_principal" in sub_df.columns else 0
        rate_d1 = (bal_d1 / ov_d1) if ov_d1 > 0 else None # This is DPD1 Rate (Overdue Rate), not Recovery
        
        # DPD5 (d6): T+6
        mask_d5 = (today - sub_df["due_date"]).dt.days >= 6
        ov_d5 = sub_df.loc[mask_d5, "overdue_principal"].sum() # Or owing? Usually DPD5 is / Owing
        owing_d5 = sub_df.loc[mask_d5, "owing_principal"].sum()
        bal_d5 = sub_df.loc[mask_d5, "d6_principal"].sum() if "d6_principal" in sub_df.columns else 0
        ...
        return {
            ...
            "dpd1_rate": rate_d1, # DPD1 (After 1 day)
            "dpd5_rate": rate_d5  # DPD5
        }
        
    curr_metrics = calc_aggs(df_curr, "Current MTD")
    prev_metrics = calc_aggs(df_prev, "Last Month MTD")
